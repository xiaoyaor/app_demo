...
if (!function_exists('getSidebar')) {
    ...
    function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        //$ruleList = Collection(\addons\auth\app\admin\model\AuthRule::where('status', 'normal')->where('ismenu', 1)->order('weigh', 'desc')->cache("__menu__")->select())->toArray();
//        $indexRuleList = \addons\auth\app\admin\model\AuthRule::where('status', 'normal')
//            ->where('ismenu', 0)
